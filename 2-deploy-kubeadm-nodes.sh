...
multipass exec ${NODE} -- bash -c 'sudo curl -fsSL https://get.docker.com -o get-docker.sh'
multipass exec ${NODE} -- bash -c 'sudo sh get-docker.sh'
# Setup daemon.
...
multipass exec ${NODE} --  bash -c 'sudo apt-get install -y kubelet=1.17.8-00 kubeadm=1.17.8-00 kubectl=1.17.8-00'
